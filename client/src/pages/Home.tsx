...
import { Layout, Card, Button, Form, Input, DatePicker, message, Modal, Select } from 'antd';
...
export default function Home() {
  ...
  // ====== Cập nhật thông tin ======
  const handleUpdateInfo = async (values: { fullname: string; phone: string; email: string; gender: string }) => {
    if (!user) return;
    try {
      const updatedUser = { ...user, ...values, gender: values.gender === 'Male' };
  ...
  const handleSaveBudget = async () => {
    ...
    try {
      ...
      const existingMonth = user.monthlyCategories?.find(
        (item) => dayjs(item.month).format('YYYY-MM') === selectedMonth.format('YYYY-MM')
      );
      ...
      if (existingMonth) {
        ...
        const updatedMonthlyCategories = user.monthlyCategories.map((item) =>
          item.id === existingMonth.id ? { ...item, categories: updatedCategories } : item
        );
  ...
  return (
    <Layout>
      <Layout>
        <Header/>
        <Layout style={{ padding: '24px', background: '#f0f2f5',alignContent:"center"}}>
          <Sidebar />
          <Content>
            <div style={{ marginBottom: 16,backgroundColor:"#4F46E5",width:"50%",padding:"15px",color:"white",borderRadius:"10px",paddingLeft:"13%",marginLeft:"25%",}}>
              <h2 style={{ fontSize: '24px'}}>
                🎯 Kiểm soát chi tiêu thông minh
              </h2>
              <p style={{paddingLeft:"27px"}}>Theo dõi ngân sách và thu chi hằng tháng dễ dàng</p>
            </div>
            <p style={{ fontSize: '30px',padding:"10px",paddingLeft:"37%"}}>
            ...
            <br />
            <Card style={{paddingLeft:"16%",width:"50%",marginLeft:"25%"}}>
              📅 Chọn tháng:
              <DatePicker
                  picker="month"
                  format="MMMM YYYY"
            ...
            <br />
            <Card style={{paddingLeft:"12%",width:"50%",marginLeft:"25%"}}>
              💰 Ngân sách tháng:
              <Input
                  type="number"
                ...
                />
                <Button type="primary" onClick={handleSaveBudget}>Lưu</Button>
            </Card>
            <br />
            {/* ==== Thông tin cá nhân ==== */}
            <div style={{marginLeft:"25%"}}>
              <h2 style={{ fontSize: '20px',color:"#4F46E5",paddingLeft:"23%"}}>Quản Lý Thông tin cá nhân</h2>
                <Form layout="vertical" style={{ marginTop: '20px',display:"flex",gap:"10px"}}>
                    <div>
                      <Form.Item label="Name">
                        <Input value={user?.fullname || ''} style={{width:"400px"}} />
                      </Form.Item>
                      <Form.Item label="Email">
                        <Input value={user?.email || ''} />
                      </Form.Item>
                      <Form.Item>
                        <Button type="primary" onClick={() => setIsEditModalVisible(true)} style={{ marginRight: '8px',width:"400px",borderColor:"#4F46E5" ,backgroundColor:"white",color:"#4F46E5"}}>
                          Change Information
                        </Button>
                      </Form.Item>
                    </div>
                    <div>
                      <Form.Item label="Phone">
                        <Input value={user?.phone || '' }  style={{width:"400px"}} />
                      </Form.Item>
                      <Form.Item label="Gender">
                        <Input value={user?.gender ? 'Male' : 'Female'} />
                      </Form.Item>
                      <Form.Item>                 
                        <Button type="primary" onClick={() => setIsPasswordModalVisible(true)} style={{width:"400px",borderColor:"#4F46E5" ,backgroundColor:"white",color:"#4F46E5"}}>
                          Change Password
                        </Button>
                      </Form.Item>
                    </div>
            ...
            >
              ...
              >
                <Form.Item label="Họ tên" name="fullname" rules={[{ required: true, message: 'Không được để trống!' }]}>
                  <Input />
                </Form.Item>
